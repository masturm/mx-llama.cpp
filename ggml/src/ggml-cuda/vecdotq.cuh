...
// Sign-mask LUT shared by the IQ decode kernels: byte j is 0xFF iff bit j of
// the index is set. Turns an 8-bit sign byte into per-value 0x00/0xFF masks.
...
#pragma unroll
    for (int k0 = 0; k0 < 8; k0 += 2) {
        const uint2 grid = ((const uint2*)iq2xxs_grid)[aux8[k0/2]];
        ...
        // ~grid+1 is per-byte -grid; safe because no iq2xxs_grid byte is 0
        const int grid0 = (grid.x & ~mask.x) | ((~grid.x + 0x01010101u) & mask.x);
        const int u0 = get_int_b4(bq8_1[iqs/2].qs, k0 + 0);
        sumi = ggml_cuda_dp4a(grid0, u0, sumi);

        const int grid1 = (grid.y & ~mask.y) | ((~grid.y + 0x01010101u) & mask.y);
